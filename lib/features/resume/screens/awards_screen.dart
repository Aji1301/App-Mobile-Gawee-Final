...
class _AwardsScreenState extends State<AwardsScreen> {
  ...
  void _loadExistingData() async {
    ...
    if (mounted && data != null) {
      // Mengambil data dari kolom 'awards'
      if (data['awards'] != null) {
        try {
          final awardData = data['awards'] is String 
              ? jsonDecode(data['awards']) 
              : data['awards'];

          setState(() {
            _titleController.text = awardData['title'] ?? '';
            _descriptionController.text = awardData['description'] ?? '';
            
            if (_months.contains(awardData['start_month'])) _startMonth = awardData['start_month'];
            if (_years.contains(awardData['start_year'])) _startYear = awardData['start_year'];
          });
        } catch (e) {
          print("Error parsing awards: $e");
        }
      }
    }
    ...
    if (mounted) {
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Award saved!")),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          child: Column(
            ...
            children: [
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.military_tech_outlined, color: primaryColor, size: 40),
                    ),
              ...
              if (_isLoading)
                ...
                _buildTextField("e.g. Employee of the Month", _titleController, cardColor, textColor, hintColor),
                const SizedBox(height: 16),

                // Start Date (Date Received)
                _buildFormLabel("Date Received", textColor),
                Row(
                  children: [
  ...
  Widget _buildDescriptionTextField(Color bgColor, Color textColor, Color hintColor) {
    return Container(
      ...
      child: TextField(
        ...
        style: GoogleFonts.poppins(color: textColor),
        decoration: InputDecoration(
          hintText: "Description about the award...",
          hintStyle: GoogleFonts.poppins(color: hintColor),
          border: InputBorder.none,
