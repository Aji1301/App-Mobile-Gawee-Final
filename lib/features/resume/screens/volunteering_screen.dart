...
class _VolunteeringScreenState extends State<VolunteeringScreen> {
  ...
  void _loadExistingData() async {
    ...
    if (mounted && data != null) {
      // Mengambil data dari kolom 'volunteering'
      if (data['volunteering'] != null) {
        try {
          final volData = data['volunteering'] is String 
              ? jsonDecode(data['volunteering']) 
              : data['volunteering'];

          setState(() {
            _titleController.text = volData['title'] ?? '';
            _descriptionController.text = volData['description'] ?? '';
            
            if (_months.contains(volData['start_month'])) _startMonth = volData['start_month'];
            if (_years.contains(volData['start_year'])) _startYear = volData['start_year'];
            if (_months.contains(volData['end_month'])) _endMonth = volData['end_month'];
            if (_years.contains(volData['end_year'])) _endYear = volData['end_year'];
          });
        } catch (e) {
          print("Error parsing volunteering: $e");
        }
      }
    }
    ...
    if (mounted) {
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Volunteering saved!")),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          child: Column(
            ...
            children: [
                ...
                child: Column(
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.volunteer_activism_outlined, color: primaryColor, size: 40),
                    ),
              ...
              if (_isLoading)
                ...
                _buildTextField("Title", _titleController, cardColor, textColor, hintColor),
                const SizedBox(height: 16),
